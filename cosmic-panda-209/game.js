...
const FOG_COLOR = new THREE.Color(0x4a2a44);   // dusky mauve haze between the pink horizon and the dark zenith
...
// moon direction is recomputed every frame from the track's heading (see
// updateMoonDir) so the moon always sits ahead on the horizon, no matter
// which way the circuit is currently winding
const MOON_ELEV = 0.16;   // radians above horizon — low, so it reads as "on the horizon"
const MOON_DIR = new THREE.Vector3(0, Math.sin(MOON_ELEV), Math.cos(MOON_ELEV));
// inlined (not mkFrame()) — mkFrame isn't defined until later in the file and this runs at module top-level
const moonFrame = { p: new THREE.Vector3(), t: new THREE.Vector3(), l: new THREE.Vector3(), k: 0 };
const moonTmp = new THREE.Vector3();
function updateMoonDir() {
  frame(player.s + 60, moonFrame);   // look a bit ahead along the track for stability
  moonTmp.set(moonFrame.t.x, 0, moonFrame.t.z).normalize();
  const c = Math.cos(MOON_ELEV);
  MOON_DIR.set(moonTmp.x * c, Math.sin(MOON_ELEV), moonTmp.z * c);
  moonLight.position.copy(MOON_DIR).multiplyScalar(200);
}

const moonLight = new THREE.DirectionalLight(0xffb070, 1.4);   // warm, low sunset sun
moonLight.position.copy(MOON_DIR).multiplyScalar(200);
scene.add(moonLight);
// soft fill from roughly the opposite side, so shaded panels aren't as contrasty
// against the lit side — raises the shadow floor and hides normal banding
const fillDir = new THREE.Vector3(-0.45, 0.35, 0.6).normalize();
const fillLight = new THREE.DirectionalLight(0x8a7ad0, 0.55);
fillLight.position.copy(fillDir).multiplyScalar(200);
scene.add(fillLight);
// pink dusk from above, warm bounce from below — brighter than the old night rig
scene.add(new THREE.HemisphereLight(0x9a5570, 0x2c1a24, 1.6));

// ---------------------------------------------------------- sky
// sunset sky: orange at the horizon, pink through the middle, dark blue at the zenith
const timeU = { value: 0 };   // shared clock for sky + grid shaders
const SKY_GLSL = `
vec3 skyGrad(vec3 d, vec3 sd){
  float h = clamp(d.y, -0.05, 1.0);
  vec3 zen = vec3(0.012, 0.018, 0.070);
  vec3 mid = vec3(0.360, 0.095, 0.150);
  vec3 hor = vec3(0.700, 0.220, 0.055);
  vec3 col = mix(hor, mid, smoothstep(0.0, 0.26, h));
  col = mix(col, zen, smoothstep(0.18, 0.70, h));
  // extra warmth banked around the sun's side of the horizon
  float s = max(dot(normalize(vec3(d.x, 0.0, d.z)), normalize(vec3(sd.x, 0.0, sd.z))), 0.0);
  col += vec3(0.30, 0.10, 0.02) * pow(s, 3.0) * (1.0 - smoothstep(0.0, 0.45, h));
  return col;
}`;

const skyMat = new THREE.ShaderMaterial({
  side: THREE.BackSide, depthWrite: false, fog: false,
  uniforms: { sunDir: { value: MOON_DIR }, time: timeU },
  vertexShader: `
    varying vec3 vDir;
  ...
  fragmentShader: SKY_GLSL + `
    ...
    void main(){
      ...
      // giant, soft-edged setting sun low on the horizon
      float disc = smoothstep(0.930, 0.945, s);
      col += vec3(1.00, 0.62, 0.30) * disc * 1.1;
      col += vec3(1.00, 0.45, 0.15) * pow(s, 24.0) * 0.30;   // warm close-in glow
...
// bake the sky into an environment map so metallic paint picks up the sunset
{
  ...
  const rt = pmrem.fromScene(envScene, 0.25, 1, 6500);   // blur the reflection bake so it stays soft on glossy paint
...
// warm halo behind the sun disc — kept soft so it reads as haze, not glare
const moonSprite = new THREE.Sprite(new THREE.SpriteMaterial({
  map: glowTex, color: 0xff9a55, transparent: true, opacity: 0.34,
  blending: THREE.AdditiveBlending, depthWrite: false, fog: false
}));
moonSprite.scale.set(900, 900, 1);
scene.add(moonSprite);

// ---------------------------------------------------------- neon grid floor
// Tron-style plane: dark gray ground with glowing teal grid lines
...
const gridMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      vec3 base = vec3(0.035, 0.035, 0.04);         // dark dark gray ground
      base = mix(base, vec3(0.09, 0.09, 0.10), clamp(vWp.y / 70.0, 0.0, 1.0));  // lighter peaks
      vec3 neon = vec3(0.10, 0.95, 0.85) * pulse;   // teal grid lines
...
const TRACK = (() => {
  ...
  for (let i = 0; i < CPN; i++) {
    ...
    const y = 18 + 8 * Math.sin(a * 2 + 1.2) + 5 * Math.sin(a * 3 + 3.4) + 4 * Math.sin(a * 5 + 0.9);
  ...
  // gaussian jump-ramp crests added in arc-length space — sharp enough that a
  // fast car's momentum carries it off the top (the spline alone smooths any
  // control-point bump below launch curvature)
  const RAMPS = [[0.16, 6, 24], [0.37, 7, 26], [0.55, 6, 22], [0.74, 8, 28], [0.88, 6, 24]];
  const rampY = (s) => {
    let y = 0;
    for (const [f, A, w] of RAMPS) { const d = s - f * L; y += A * Math.exp(-(d * d) / (w * w)); }
    return y;
  };
  for (let i = 0; i < N; i++) {
  ...
  for (let i = 0; i < N; i++) {
    const t = new THREE.Vector3().subVectors(pos[(i + 1) % N], pos[(i - 1 + N) % N]).normalize();
    tan.push(t);
    left.push(new THREE.Vector3().crossVectors(up, new THREE.Vector3(t.x, 0, t.z).normalize()).normalize());
  }
  for (let i = 0; i < N; i++) {
...
const LED_COLOR = 0x36e6ff;   // icy cyan — shared by the strip, road-edge glow, and tracking lights
...
const roadTex = canvasTex(512, 512, (g, w, h) => {
  ...
  // (solid white edge lines removed — the raised kerb lip is now a glowing LED strip)
  // dashed center line
...
// emissive map baked into the road: cyan spill that fades inward from each edge,
// so the road appears lit by the LED strip along its ENTIRE length (an emissive
// strip alone can't cast light in WebGL — this fakes the bounce consistently)
const roadGlowTex = canvasTex(512, 512, (g, w, h) => {
  g.fillStyle = '#000'; g.fillRect(0, 0, w, h);
  const glowW = w * 0.05;
  const gL = g.createLinearGradient(0, 0, glowW, 0);
  gL.addColorStop(0, 'rgba(255,255,255,1)'); gL.addColorStop(1, 'rgba(255,255,255,0)');
  g.fillStyle = gL; g.fillRect(0, 0, glowW, h);
  const gR = g.createLinearGradient(w, 0, w - glowW, 0);
  gR.addColorStop(0, 'rgba(255,255,255,1)'); gR.addColorStop(1, 'rgba(255,255,255,0)');
  g.fillStyle = gR; g.fillRect(w - glowW, 0, glowW, h);
});
roadGlowTex.wrapT = THREE.RepeatWrapping;
...
const road = new THREE.Mesh(ribbon(-HALF_W, HALF_W, 0, 13),
  new THREE.MeshStandardMaterial({
    map: roadTex, roughness: 0.94, metalness: 0.0,
    emissive: LED_COLOR, emissiveMap: roadGlowTex, emissiveIntensity: 1.9   // edge glow from the LED strip
  }));
scene.add(road);
...
// ---------------------------------------------------------- LED edge strips
// The raised inner lip of the warning zone is a bright glowing LED strip that
// runs the full length of both sides of the track. Two point lights track the
// car along these strips so the paint actually catches the glow.
const ledMat = new THREE.MeshStandardMaterial({
  color: 0x03151b, emissive: LED_COLOR, emissiveIntensity: 4.5,
  roughness: 0.35, metalness: 0.0, toneMapped: false   // keep the strip reading as a pure light source, not surface-lit
});
// a thin ribbon sitting proud of the road, right on the inner edge of each kerb
scene.add(new THREE.Mesh(ribbon(-HALF_W - 0.10, -HALF_W + 0.14, 0.11, 3.4), ledMat));
scene.add(new THREE.Mesh(ribbon(HALF_W - 0.14, HALF_W + 0.10, 0.11, 3.4), ledMat));

// lights that ride along the strips beside the car so it's lit from both edges
const LED_LIGHT_DIST = 26, LED_LIGHT_INT = 90;
const ledLightL = new THREE.PointLight(LED_COLOR, LED_LIGHT_INT, LED_LIGHT_DIST, 2.0);
const ledLightR = new THREE.PointLight(LED_COLOR, LED_LIGHT_INT, LED_LIGHT_DIST, 2.0);
scene.add(ledLightL, ledLightR);
const ledFrame = mkFrame();
function updateLedLights() {
  frame(player.s, ledFrame);
  const p = ledFrame.p, l = ledFrame.l;
  ledLightL.position.set(p.x - l.x * HALF_W, p.y + 0.5, p.z - l.z * HALF_W);
  ledLightR.position.set(p.x + l.x * HALF_W, p.y + 0.5, p.z + l.z * HALF_W);
}
...
// ---------------------------------------------------------- synthwave mountains
// Value-noise heightfield rendered with the same neon-grid shader as the
// floor plane. A corridor around the track is flattened to ROAD_CLEAR metres
// below the local road height so hills never stab through the deck.
...
function landH(x, z) {
  const big = (fbm(x * 0.0011 + 11.3, z * 0.0011 - 4.1) - 0.48) * 120;  // mountain ranges
  const hills = (fbm(x * 0.005 - 7.7, z * 0.005 + 2.9) - 0.5) * 30;     // rolling hills
  return Math.max(big + hills, -6);   // clamp valleys into flat dark plains
}
const ROAD_CLEAR = 4;
{
  const HALF = 1800, GN = 300, CORRIDOR = 26;
  const geo = new THREE.PlaneGeometry(HALF * 2, HALF * 2, GN, GN);
  geo.rotateX(-Math.PI / 2);
  const pa = geo.attributes.position;
  const tp = [];
  for (let i = 0; i < TRACK.N; i += 6) tp.push(TRACK.pos[i]);
  for (let i = 0; i < pa.count; i++) {
    ...
    for (let k = 0; k < tp.length; k++) {
      ...
      if (d2 < minD2) { minD2 = d2; ny = tp[k].y; }
    }
    const dist = Math.sqrt(minD2);
    if (dist < CORRIDOR + 50) {
      const cap = ny - ROAD_CLEAR;
      const t = clamp((dist - CORRIDOR) / 50, 0, 1);
      h = Math.min(h, lerp(cap, Math.max(h, cap), t));
    }
    pa.setY(i, h);
  }
  const terrain = new THREE.Mesh(geo, gridMat);
    ...
    new THREE.GLTFLoader().parse(buf.buffer, '', (gltf) => {
      ...
      model.traverse((o) => {
        if (o.isMesh && o.material && /^BodyMain/.test(o.material.name)) {
          ...
          o.material.roughness = Math.max(Math.min(o.material.roughness, 0.32), 0.28);
        }
...
class SFX {
  ...
  init() {
    ...
    this.ahhBuf = null;
    this.watchItBuf = null;
    decodeSample(window.AHH_M4A_B64, 'ahh', (buf) => { this.ahhBuf = buf; });
    decodeSample(window.WATCHIT_M4A_B64, 'watch it', (buf) => { this.watchItBuf = buf; });
    this.oopsBuf = null;
    decodeSample(window.OOPS_M4A_B64, 'oops', (buf) => { this.oopsBuf = buf; });
    this.goBuf = null;
    decodeSample(window.GO_M4A_B64, 'go', (buf) => { this.goBuf = buf; });
  ...
  ahh() { this.playSample(this.ahhBuf, 0.9); }
  watchIt() { this.playSample(this.watchItBuf, 0.9); }
  oops() { this.playSample(this.oopsBuf, 0.9); }
  raceGo() { if (!this.playSample(this.goBuf, 1.0)) this.go(); }   // the Go-eevee-go voice line
...
// ---------------------------------------------------------- game state
const MAX_SPEED = 62;
const JUMP_G = 40;              // gravity for airborne flight off crests (m/s^2) — heavier, so crest air is short and controlled
const JUMP_VY = 12;             // takeoff speed for a deliberate jump (m/s) — just enough hang time to clear a cone
const CREST_LAUNCH_DAMP = 0.5;  // halves the vertical velocity a hill crest launches the car with
const EXTREME_SPEED = MAX_SPEED + 55;    // giant-coin / backflip boost ceiling
const FALL_D = HALF_W + 1.0;   // a forgiving margin past the kerb
...
let stateT = 0;
let countShown = -1;
const INTRO_HOLD = 2.0;   // seconds to hold on the front-of-car shot before the 3-2-1 countdown/sweep starts
let runCoins = 0, runWrecks = 0;
let finishReason = null;      // 'finish' | 'fall'
let fireCooldown = 0, beamT = 0, shake = 0, bumpCd = 0, rearGlowT = 0, kerbAhhCd = 0;
let splashDone = false;
let popT = -1;                // "POP!" prompt clock (-1 = hidden)
let popCd = 4 + rng() * 6;    // countdown to next random POP! prompt

const keys = { left: false, right: false, fire: false, up: false, orbitL: false, orbitR: false };
let prevJumpInput = false;   // edge-detect the jump input so holding it doesn't chain jumps
...
// swap the procedural red cars for the Mini Cooper S model once it loads
(function loadMiniModel() {
  if (!window.MINI_GLB_B64 || !THREE.GLTFLoader) return;
  try {
    const bin = atob(window.MINI_GLB_B64);
    const buf = new Uint8Array(bin.length);
    for (let i = 0; i < bin.length; i++) buf[i] = bin.charCodeAt(i);
    new THREE.GLTFLoader().parse(buf.buffer, '', (gltf) => {
      const proto = gltf.scene;
      const box = new THREE.Box3().setFromObject(proto);
      proto.scale.setScalar(4.1 / (box.max.z - box.min.z));   // opponent-car size
      for (const o of opps) {
        const wasVisible = o.mesh.visible;
        scene.remove(o.mesh);
        const m = proto.clone(true);
        m.userData.spin = [];
        m.traverse((c) => { if (/Wheel$/.test(c.name)) m.userData.spin.push(c); });
        m.visible = wasVisible;
        scene.add(m);
        o.mesh = m;
      }
    }, (e) => { console.warn('mini model failed, keeping built-in cars', e); });
  } catch (e) { console.warn('mini model failed, keeping built-in cars', e); }
})();
...
window.addEventListener('keydown', (e) => {
  if (e.code === 'ArrowLeft' || e.code === 'KeyA') {
    if (e.shiftKey) { keys.orbitL = true; keys.left = false; camOrbitReset = false; }
    else keys.left = true;
    e.preventDefault();
  }
  else if (e.code === 'ArrowRight' || e.code === 'KeyD') {
    if (e.shiftKey) { keys.orbitR = true; keys.right = false; camOrbitReset = false; }
    else keys.right = true;
    e.preventDefault();
  }
  else if (e.code === 'ArrowDown' || e.code === 'KeyS') {
    if (e.shiftKey) { camOrbitReset = true; keys.orbitL = keys.orbitR = false; }
    e.preventDefault();
  }
...
window.addEventListener('keyup', (e) => {
  if (e.code === 'ArrowLeft' || e.code === 'KeyA') { keys.left = false; keys.orbitL = false; }
  else if (e.code === 'ArrowRight' || e.code === 'KeyD') { keys.right = false; keys.orbitR = false; }
...
// the laser and a double tap triggers a jump (timed, since taps can't hold)
let touchSteer = 0;      // analog steer from the joystick, +1 = car-left
let touchWheelieT = 0;   // seconds of double-tap jump pulse remaining
...
window.addEventListener('pointerup', (e) => {
  ...
  if (now - lastTapT < 350) { touchWheelieT = 0.85; lastTapT = -1e9; }   // double tap: jump
  else lastTapT = now;
});
...
function resetRun() {
  ...
  player.air = false; player.vy = 0; player.y = 0;
  ...
  camSnap = true; camD = 0;
  camOrbit = 0; camOrbitReset = false;
}
...
function showCount(n) {
  ...
  if (n === 0) sfx.raceGo(); else sfx.beep();
}

function fallOff() {
  state = 'fall';
  stateT = 0;
  player.fallVy = player.air ? player.vy : 2.5;
  player.fallY = player.air ? Math.max(player.y - player.frame.p.y, 0) : 0;
  player.air = false;
  sfx.rumbleLevel(0);
  sfx.crash();
}
...
let camD = 0;
let camSnap = true;
let camOrbit = 0;          // manual orbit angle around the car (0 = default rear chase)
let camOrbitReset = false; // easing back to the rear view after shift+down
...
function updatePlayer(dt) {
  ...
  // --- jump input (edge-triggered: a held key or touch pulse only fires once) ---
  const jumpInput = (keys.up || touchWheelieT > 0) && player.flipT < 0;
  const jumpPressed = jumpInput && !prevJumpInput;
  prevJumpInput = jumpInput;
  // jumping while the "POP!" prompt is up triggers a backflip + boost instead of a plain jump
  if (jumpPressed && popT >= 0 && player.flipT < 0) {
    player.flipT = 0;
    // successful POP flip -> same kick as a giant coin: instant extreme speed
    player.extremeT = Math.max(player.extremeT, 4.5);
    player.speed = EXTREME_SPEED;
    popT = -1;
    el.popText.classList.remove('show');
    sfx.wee();
  } else if (jumpPressed && !player.air) {
    player.air = true;
    player.vy = JUMP_VY;   // hop just high enough to clear a cone
  }
  if (player.flipT >= 0) {
    player.flipT += dt;
    const FDUR = 0.9;
    ...
    player.wheelieP = damp(player.wheelieP, 0, 10, dt);
  }
  ...
  if (Math.abs(player.d) > HALF_W * 0.82 && Math.sign(steerAcc) === Math.sign(player.d)) steerAcc *= 0.55;
  // centrifugal drift in corners (the core challenge)
  const centrifugal = -f.k * player.speed * player.speed * 0.30;
  // gentle self-centering
  const springK = Math.abs(player.d) > HALF_W * 0.78 ? 2.6 : 0.75;
  ...
  // riding the red/white kerb: yelp on entry, re-yelp every 1.5s while on it
  kerbAhhCd -= dt;
  if (Math.abs(player.d) > HALF_W - 0.8) {
    if (kerbAhhCd <= 0) { sfx.ahh(); kerbAhhCd = 1.5; }
  } else if (kerbAhhCd > 0.3) {
    kerbAhhCd = 0.3;   // short off-kerb grace so weaving re-triggers quickly but not instantly
  }
  ...
  // --- place car ---
  frame(player.s, f);
  // vertical: glued to the road until a crest drops away faster than gravity,
  // then ballistic flight until the road comes back up to meet the car
  const roadY = f.p.y;
  if (!player.air) {
    const ballY = player.y + player.vy * dt - 0.5 * JUMP_G * dt * dt;
    if (ballY > roadY + 0.001 && player.speed > 18) {
      player.air = true;                       // launched off a crest
      // dampen only the momentum actually carried into the launch, not the
      // per-frame road-tangent velocity — that has to stay accurate or the
      // crest-detection above misfires on ordinary slopes and the car jitters
      const vy0 = player.vy * CREST_LAUNCH_DAMP;
      player.y = player.y + vy0 * dt - 0.5 * JUMP_G * dt * dt;
      player.vy = vy0 - JUMP_G * dt;
    } else {
      player.y = roadY;
      player.vy = f.t.y * player.speed;        // vertical velocity the road imparts
    }
  } else {
    player.vy -= JUMP_G * dt;
    player.y += player.vy * dt;
    if (player.y <= roadY) {                   // touchdown
      if (player.vy < -7) { shake = Math.min(shake + clamp(-player.vy * 0.03, 0, 0.5), 0.8); sfx.bump(); }
      player.air = false;
      player.y = roadY;
      player.vy = f.t.y * player.speed;
    }
  }
  playerCar.position.set(f.p.x + f.l.x * player.d, player.y, f.p.z + f.l.z * player.d);
  const airOff = player.air ? player.y - roadY : 0;   // height above the road while flying
  ...
  // in the air, pitch follows the flight path instead of the road
  const pitchY = player.air ? clamp(player.vy / Math.max(player.speed, 1), -1, 1) : f.t.y;
  playerCar.rotation.set(-Math.asin(clamp(pitchY, -1, 1)) * 0.9, yawTrack + player.yawVis, -player.yawVis * 0.55, 'YXZ');
  // backflip pitch about the car's lateral axis, plus a hop during the flip
  ...
  for (const c of coins) {
    ...
    if (Math.abs(ds) < 2.6 && Math.abs(c.d - player.d) < 1.7 && airOff < 2.5) {
  ...
  for (const gc of giantCoins) {
    ...
    if (Math.abs(ds) < 4.5 && Math.abs(gc.d - player.d) < 4.8 && airOff < 5.5) {
  ...
  for (const cn of cones) {
    ...
    if (ds < -TRACK.L / 2) ds += TRACK.L;
    if (ds > TRACK.L / 2) ds -= TRACK.L;
    if (Math.abs(ds) < 2.4 && Math.abs(cn.d - player.d) < 1.55 && airOff < 1.2) {
  ...
  if (bumpCd <= 0) {
    for (const o of opps) {
      ...
      if (Math.abs(ds) < 4.3 && Math.abs(o.d - player.d) < 2.05 && airOff < 1.6) {
        player.speed *= 0.55;
...
function updateFall(dt) {
  ...
  const groundY = f.p.y - ROAD_CLEAR;
  if (playerCar.position.y < groundY + 0.6 && !splashDone) {
    splashDone = true;
    spawnBurst(playerCar.position.x, playerCar.position.y + 0.4, playerCar.position.z,
      { color: 0xff7ad8, speed: 12, up: 12, size: 1.9, dur: 1.3, gravity: 22 });
    spawnBurst(playerCar.position.x, playerCar.position.y + 0.1, playerCar.position.z,
      { color: 0xc95bb8, speed: 7, up: 8, size: 1.4, dur: 1.0, gravity: 16 });
    sfx.splash();
  }
  if (playerCar.position.y < groundY - 2.5) {
...
function updateOpponents(dt) {
  for (const o of opps) {
    if (o.state === 'alive') {
      ...
      if (o.mesh.userData.spin) for (const w of o.mesh.userData.spin) w.rotation.x += o.speed * dt * 1.15;
      else for (const w of o.mesh.userData.wheels) w.children[0].rotation.x += o.speed * dt / 0.3 * 0.35;
      ...
      const oGroundY = o.frame.p.y - ROAD_CLEAR;
      if (o.mesh.position.y < oGroundY + 0.6 && !o.splashed) {
        o.splashed = true;
        spawnBurst(o.mesh.position.x, o.mesh.position.y + 0.3, o.mesh.position.z,
          { color: 0xff7ad8, speed: 9, up: 9, size: 1.5, dur: 1.0, gravity: 20 });
        sfx.splash();
      }
      if (o.mesh.position.y < oGroundY - 3) {
        o.state = 'respawning';
...
function updateCamera(dt) {
  ...
  if (state === 'count') {
    // cinematic reveal: hold on the front of the car, then sweep 180°
    // around it to land in the chase position (behind, looking at the rear)
    const k = clamp((stateT - INTRO_HOLD) / 3.4, 0, 1);
    const ease = 1 - Math.pow(1 - k, 3);
    const ang = (1 - ease) * Math.PI;
    ...
    const dist = 8.26 - 1.76 * (1 - ease), height = 3.36 - 1.16 * (1 - ease);
    ...
    if (state === 'fall' || state === 'over') {
      ...
      camLook.y += 2.1;
      const airOff = player.air ? player.y - player.frame.p.y : 0;
      camPos.y += airOff * 0.6;
      camera.position.copy(camPos);
      camLook.y += airOff * 0.8;
    }
    // shift+left/right orbits around the car; shift+down eases back to the rear
    if (keys.orbitL) camOrbit += 1.8 * dt;
    if (keys.orbitR) camOrbit -= 1.8 * dt;
    if (camOrbitReset) {
      camOrbit = damp(camOrbit, 0, 8, dt);
      if (Math.abs(camOrbit) < 0.005) { camOrbit = 0; camOrbitReset = false; }
    }
    const orbiting = Math.abs(camOrbit) > 0.001;
    if (orbiting && state !== 'fall' && state !== 'over') {
      const cx = playerCar.position.x, cz = playerCar.position.z;
      const c = Math.cos(camOrbit), s = Math.sin(camOrbit);
      const ox = camPos.x - cx, oz = camPos.z - cz;
      camPos.x = cx + ox * c - oz * s;
      camPos.z = cz + ox * s + oz * c;
      camera.position.copy(camPos);
      camLook.set(cx, playerCar.position.y + 1.6, cz);
    }
    camSnap = false;
    camera.lookAt(camLook);
    if (!orbiting) camera.rotation.z += -player.yawVis * 0.25;
  }
  ...
  moonSprite.position.copy(camera.position).addScaledVector(MOON_DIR, 2900);
  ocean.position.set(Math.round(camera.position.x / 20) * 20, -7, Math.round(camera.position.z / 20) * 20);
}
...
function step(dt) {
  ...
  if (state === 'count') {
    const ct = stateT - INTRO_HOLD;   // countdown clock, held at the front shot until INTRO_HOLD elapses
    if (ct >= 0) {
      const n = 3 - Math.floor(ct);
    ...
    // sit still on the line during the intro — only starts rolling once the race begins
    player.speed = 0;
    const f = frame(player.s, player.frame);
    playerCar.position.set(f.p.x + f.l.x * player.d, f.p.y, f.p.z + f.l.z * player.d);
    playerCar.rotation.set(0, Math.atan2(f.t.x, f.t.z), 0);
    sfx.engine(0.15 + (3 - Math.max(ct, 0)) * 0.04, 0.4);
  } else if (state === 'race') {
    updatePlayer(dt);
    if (state === 'race') {
