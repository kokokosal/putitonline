...
const gridMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      base = mix(base, vec3(0.11, 0.035, 0.22), clamp(vWp.y / 70.0, 0.0, 1.0));  // lighter peaks
      // neon hue cycles pink -> teal -> yellow -> purple over 10 s
      vec3 c0 = vec3(1.00, 0.16, 0.72);   // hot pink
      vec3 c1 = vec3(0.10, 0.95, 0.85);   // teal
      vec3 c2 = vec3(1.00, 0.85, 0.20);   // yellow
      vec3 c3 = vec3(0.62, 0.25, 0.95);   // purple
      float ph = fract(time / 10.0) * 4.0;
      vec3 hue = mix(c0, c1, smoothstep(0.0, 1.0, ph));
      hue = mix(hue, c2, smoothstep(1.0, 2.0, ph));
      hue = mix(hue, c3, smoothstep(2.0, 3.0, ph));
      hue = mix(hue, c0, smoothstep(3.0, 4.0, ph));
      vec3 neon = hue * pulse;
...
// ---------------------------------------------------------- game state
const MAX_SPEED = 62;
const JUMP_G = 24;              // gravity for airborne flight off crests (m/s^2)
