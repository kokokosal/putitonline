...
// start gate
{
  ...
  const banTex = canvasTex(512, 96, (g, w, h) => {
    g.fillStyle = '#181226'; g.fillRect(0, 0, w, h);
    g.fillStyle = '#ffb45e';
    g.font = 'italic 900 54px "Avenir Next", "Segoe UI", sans-serif';
    g.textAlign = 'center'; g.textBaseline = 'middle';
    g.fillText('SUNSET RUSH', w / 2, h / 2 + 2);
  });
...
for (const loop of loops) {
  ...
  const SEG = 90;
  // per-θ frame helpers
  const cpt = new THREE.Vector3(), nin = new THREE.Vector3(), tmpc = new THREE.Vector3();
  const pointAt = (th, out) => out.copy(b.p).addScaledVector(b.up, LOOP_R * (1 - Math.cos(th))).addScaledVector(b.fwd, LOOP_R * Math.sin(th));
  ...
  // --- inner drivable band (road texture, faces the loop centre) ---
  {
    ...
    for (let i = 0; i <= SEG; i++) {
      ...
      pointAt(th, cpt);
      const o = i * 6;
      posA[o] = cpt.x - b.left.x * LOOP_W; posA[o + 1] = cpt.y - b.left.y * LOOP_W; posA[o + 2] = cpt.z - b.left.z * LOOP_W;
      posA[o + 3] = cpt.x + b.left.x * LOOP_W; posA[o + 4] = cpt.y + b.left.y * LOOP_W; posA[o + 5] = cpt.z + b.left.z * LOOP_W;
  ...
  // --- structural shell: a rectangular tube giving the track real thickness ---
  {
    ...
    for (let i = 0; i <= SEG; i++) {
      ...
      const IL = cpt.clone().addScaledVector(b.left, LOOP_W).addScaledVector(nin, 0.06);
      const IR = cpt.clone().addScaledVector(b.left, -LOOP_W).addScaledVector(nin, 0.06);
      const OL = cpt.clone().addScaledVector(b.left, LOOP_W).add(outw);
      const OR = cpt.clone().addScaledVector(b.left, -LOOP_W).add(outw);
  ...
  // the loop circle lies in the fwd/up plane, so the rail torus (default XY ring)
  // must have local X=fwd, Y=up to trace that same circle
  const rm = new THREE.Matrix4().makeBasis(b.fwd, b.up, new THREE.Vector3().crossVectors(b.fwd, b.up));
  const rq = new THREE.Quaternion().setFromRotationMatrix(rm);
  for (const side of [-1, 1]) {
    const rail = new THREE.Mesh(new THREE.TorusGeometry(LOOP_R + LOOP_THICK * 0.5, 0.28, 10, 72), loopRailMat);
    rail.quaternion.copy(rq);
    rail.position.copy(b.p).addScaledVector(b.up, LOOP_R).addScaledVector(b.left, side * (LOOP_W + 0.35));
  ...
  for (const th of [Math.PI * 0.5, Math.PI * 1.5]) {
    pointAt(th, cpt);
    for (const side of [-1, 1]) {
      const topX = cpt.x + b.left.x * side * (LOOP_W + 0.2);
      const topZ = cpt.z + b.left.z * side * (LOOP_W + 0.2);
      const topY = cpt.y;
      const groundY = -2;
...
const el = {
  ...
  wrecks: document.getElementById('wreckCount'),
  dist: document.getElementById('distVal'),
  best: document.getElementById('bestVal'),
  marker: document.getElementById('carMarker'),
...
function updateBest() {
  ...
  el.best.textContent = list.length ? 'BEST ' + fmtTime(list[0].time) : '';
}
...
setTimeout(() => { el.help.style.opacity = '0.25'; }, 9000);
...
// loop-de-loop placement: a rigid vertical circle in each loop's fwd/up plane.
...
function loopPlace(loop, theta, d, out) {
  ...
  out.pos.copy(b.p)
    .addScaledVector(b.up, LOOP_R * (1 - c))
    .addScaledVector(b.fwd, LOOP_R * s)
    .addScaledVector(b.left, d);
  out.fwd.copy(b.fwd).multiplyScalar(c).addScaledVector(b.up, s).normalize();     // travel dir
...
function updatePlayer(dt) {
  ...
  const wheelieOn = keys.up && player.flipT < 0;
  ...
  // --- lateral physics ---
  const steer = (keys.left ? 1 : 0) - (keys.right ? 1 : 0);   // +d is car-left
...
let lastSpeedShown = -1, lastCoinsShown = -1, lastWrecksShown = -1, lastDistShown = -1, lastTimeShown = -1;
function updateHUD() {
  const kmh = Math.round(player.speed * 3.6);
  if (kmh !== lastSpeedShown) { el.speed.textContent = kmh; lastSpeedShown = kmh; }
  if (runCoins !== lastCoinsShown) { el.coins.textContent = runCoins; lastCoinsShown = runCoins; }
  if (runWrecks !== lastWrecksShown) { el.wrecks.textContent = runWrecks; lastWrecksShown = runWrecks; }
  const dm = Math.floor(player.dist);
  if (dm !== lastDistShown) { el.dist.textContent = dm + ' m'; lastDistShown = dm; }
  ...
  const frac = clamp(player.d / HALF_W, -1.15, 1.15);
  // +d is car-left, so the marker moves left (negative X) on screen
  el.marker.style.transform = 'translateX(' + (-frac * 158).toFixed(1) + 'px)';
  const off = Math.abs(frac);
